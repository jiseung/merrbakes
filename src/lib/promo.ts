...
export function clubWeekCode(): string {
  return (process.env.CLUB_WEEK_CODE || 'clubweek').trim();
}

export function isClubWeekCode(code: unknown): boolean {
  return typeof code === 'string' && code.trim().toLowerCase() === clubWeekCode().toLowerCase();
}
